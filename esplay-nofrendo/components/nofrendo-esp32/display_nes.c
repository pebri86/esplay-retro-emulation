...
static uint8_t getPixelNes(const uint8_t *bufs, int x, int y, int w1, int h1, int w2, int h2)
{
        ...
        case SCALE_STRETCH:
            ...
            for (y = 0; y < outputHeight; y += LINE_COUNT)
            {
                for (int i = 0; i < LINE_COUNT; ++i)
                {
                    ...
                    for (x = 0; x < outputWidth; x++)
                    {
                        line[calc_line][index++] = myPalette[getPixelNes(data, x, (y + i), NES_FRAME_WIDTH, NES_FRAME_HEIGHT, outputWidth, outputHeight)];
                    }
                }
                if (sending_line != -1)
                    send_line_finish();
