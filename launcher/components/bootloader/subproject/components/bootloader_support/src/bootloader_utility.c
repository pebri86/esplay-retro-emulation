...
static bool try_load_apps_image(bootloader_state_t *bs, esp_image_metadata_t *data)
{
    esp_err_t err;
	...
	if (fd!=-1 && appfsFdValid(fd)) {
		...
		if (bootloader_appfs_load_image(fd, data) == ESP_OK) {
            ESP_LOGI(TAG, "Loaded app from appfs at fd index 0x%x",
                     fd);
                     
            return true;
        }
		ESP_LOGE(TAG, "Booting %s failed! Falling back to launcher.", name);
	}
...
void bootloader_utility_load_boot_appfs_image(const bootloader_state_t *bs)
{
    esp_image_metadata_t image_data;
    if(try_load_apps_image(bs, &image_data)) {
        load_image(&image_data);
    } else {
        ESP_LOGE(TAG, "No bootable appfs image in the appfs partition, falling back booting from FACTORY");
