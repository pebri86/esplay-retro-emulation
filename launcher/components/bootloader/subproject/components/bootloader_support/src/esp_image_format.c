...
static esp_err_t appfs_image_load(int fd, esp_image_metadata_t *data)
{
#ifdef BOOTLOADER_BUILD
    bool do_load   = true;
    ...
    uint32_t appfs_offset = get_appfs_file_offset(fd);
    if(appfs_offset == -1) {
        return ESP_ERR_IMAGE_INVALID;
    }

    bzero(data, sizeof(esp_image_metadata_t));
    data->start_addr = appfs_offset;
    ...
    if (do_verify) {
        ...
        */
        ...
        if (verify_sha) {
            if (data->image_len > SIXTEEN_MB) {
                FAIL_LOAD("Image length %d doesn't fit in partition length %d", data->image_len, SIXTEEN_MB);
            }
...
esp_err_t bootloader_appfs_load_image(int fd, esp_image_metadata_t *data)
{
#if !defined(BOOTLOADER_BUILD)
    return ESP_FAIL;
#endif

    return appfs_image_load(fd, data);
}
