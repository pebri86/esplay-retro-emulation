...
static esp_err_t receive_file_content(httpd_req_t *req, FILE *fd, int content_len, char *scratch)
{
    int received;
    ...
    while (remaining > 0)
    {
        ...
        if (fwrite(scratch, 1, received, fd) != received)
        {
            return ESP_FAIL;
        }
        remaining -= received;
...
{
    httpd_resp_sendstr_chunk(req, "<!DOCTYPE html><html><body><div>");
    ...
    if (strcmp(dirpath, "/appfs") == 0)
    {
        httpd_resp_sendstr_chunk(req,
                                 ...
                                 "<thead><tr><th>Name</th><th>Size (KB)</th><th>Action</th></tr></thead>"
                                 "<tbody>");
        ...
        while (1)
        {
            ...
            snprintf(entrysize, sizeof(entrysize), "%d", size / 1024);
            ...
            httpd_resp_sendstr_chunk(req, "\"><button type=\"submit\" onclick=\"return confirm('Are you sure you want to uninstall this app?');\">Uninstall</button></form>");
    ...
    else
    {
        ...
        httpd_resp_sendstr_chunk(req,
                                 ...
                                 "<thead><tr><th>Name</th><th>Type</th><th>Size (KB)</th><th>Delete</th></tr></thead>"
                                 "<tbody>");
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            ...
            snprintf(entrysize, sizeof(entrysize), "%ld", (long)(entry_stat.st_size / 1024));
            ...
            httpd_resp_sendstr_chunk(req, entry->d_name);
            httpd_resp_sendstr_chunk(req, "\"><button type=\"submit\" onclick=\"return confirm('Are you sure you want to delete this file?');\">Delete</button></form>");
            httpd_resp_sendstr_chunk(req, "</td></tr>\n");
...
static esp_err_t install_apps_handler(httpd_req_t *req)
{
    ...
    if (receive_file_content(req, fd, req->content_len, buf) != ESP_OK)
    {
        fclose(fd);
        unlink("/sd/temporary");
